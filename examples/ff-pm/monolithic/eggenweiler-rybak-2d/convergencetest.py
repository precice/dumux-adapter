...
class Test:
    ...
    def _checkMeanRates(self, ratesDarcy, ratesFreeFlow):
        def checkRate(rates, key, expected):
            ...
            if m < expected:
                raise Exception(
                    "Rate {} is below expected value {}: {}".format(key, expected, m)
                )
            else:
                print(
                    'Test for "{}" successful: '
                    "computed vs expected rate {:.3f}/{:.3f}".format(key, m, expected)
                )
        ...
        for name, errValues in errors.items():
            for i in range(len(errValues) - 1):
                ...
                if not (
                    ...
                    and (errValues[i] < 1e-12 or errValues[i + 1] < 1e-12)
                    and (errValues[i] < 1e-12 or errValues[i + 1] < 1e-12)
                ):
