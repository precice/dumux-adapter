...
def div(M):
    return np.array(
        [
            sp.diff(M[0, 0], x) + sp.diff(M[0, 1], y),
            sp.diff(M[1, 0], x) + sp.diff(M[1, 1], y),
        ]
    )
...
def grad(v):
    return np.array(
        [[sp.diff(v[0], x), sp.diff(v[0], y)], [sp.diff(v[1], x), sp.diff(v[1], y)]]
    )
...
def analyticalSolutionStokes(case):
    if case == "Rybak":
        vFF = np.array(
            [
                -sp.cos(sp.pi * x) * sp.sin(sp.pi * y),
                sp.sin(sp.pi * x) * sp.cos(sp.pi * y),
            ]
        )
        ...
        vFF = np.array(
            [
                -1.0 / sp.pi * sp.exp(y) * sp.sin(sp.pi * x),
                (sp.exp(y) - sp.exp(1)) * sp.cos(sp.pi * x),
            ]
        )
