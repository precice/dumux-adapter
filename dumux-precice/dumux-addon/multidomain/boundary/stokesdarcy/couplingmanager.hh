...
class StokesDarcyCouplingManager : public StaggeredCouplingManager<MDTraits>
{
...
private:
    ...
    template<std::size_t id>
    using GridView = typename GetPropType<SubDomainTypeTag<id>,
...
#if DUMUX_VERSION_MAJOR >= 3 & DUMUX_VERSION_MINOR >= 4
    template<std::size_t id>
    using NumEqVector = Dumux::NumEqVector<PrimaryVariables<id>>;
#else
    template<std::size_t id>
    using NumEqVector =
    ...
    template<std::size_t id>
    using SubControlVolume = typename FVElementGeometry<id>::SubControlVolume;

    template<std::size_t id>
    using GridFaceVariables =
        GetPropType<SubDomainTypeTag<id>, Properties::GridFaceVariables>;
    template<std::size_t id>
    using ElementFaceVariables = typename GridFaceVariables<id>::LocalView;
    template<std::size_t id>
    using FaceVariables = typename ElementFaceVariables<id>::FaceVariables;

    template<std::size_t id>
    using GridVariables =
        typename MDTraits::template SubDomain<id>::GridVariables;
    using GridVariablesTuple =
...
public:
    ...
    using CouplingData =
        StokesDarcyCouplingData<MDTraits, StokesDarcyCouplingManager<MDTraits>>;

    //! Constructor
    StokesDarcyCouplingManager(
        std::shared_ptr<const GridGeometry<stokesIdx>> stokesFvGridGeometry,
              ...
              const SolutionVector &curSol)
    {
        ...
        if (Dune::FloatCmp::ne(stokesProblem->gravity(),
                               darcyProblem->spatialParams().gravity({})))
    ...
    void init(std::shared_ptr<const Problem<stokesIdx>> stokesProblem,
              ...
              const SolutionVector &curSol,
              const CouplingMode couplingMode =
                  CouplingMode::reconstructPorousMediumPressure)
    {
    ...
    void computeStencils()
    {
        couplingMapper_.computeCouplingMapsAndStencils(
            *this, darcyToStokesCellCenterCouplingStencils_,
    ...
    template<std::size_t i,
             class Assembler,
             std::enable_if_t<(i == stokesCellCenterIdx || i == stokesFaceIdx),
                              int> = 0>
    ...
    template<std::size_t i,
             std::enable_if_t<(i == stokesCellCenterIdx || i == stokesFaceIdx),
                              int> = 0>
    void bindCouplingContext(Dune::index_constant<i> domainI,
                             const Element<stokesCellCenterIdx> &element) const
    {
        stokesCouplingContext_.clear();

        const auto stokesElementIdx =
            this->problem(stokesIdx).gridGeometry().elementMapper().index(
                element);
        ...
        auto darcyFvGeometry =
            localView(this->problem(darcyIdx).gridGeometry());

        for (auto &&indices : darcyIndices) {
            const auto &darcyElement = this->problem(darcyIdx)
                                           .gridGeometry()
                                           .boundingBoxTree()
                                           .entitySet()
                                           .entity(indices.eIdx);
            ...
            const auto darcyElemSol =
                elementSolution(darcyElement, this->curSol()[darcyIdx],
                                this->problem(darcyIdx).gridGeometry());
            VolumeVariables<darcyIdx> darcyVolVars;
            darcyVolVars.update(darcyElemSol, this->problem(darcyIdx),
                                darcyElement, scv);
    ...
    void bindCouplingContext(Dune::index_constant<darcyIdx> domainI,
     ...
     */
    void bindCouplingContext(Dune::index_constant<darcyIdx> domainI,
                             const Element<darcyIdx> &element) const
    {
        darcyCouplingContext_.clear();

        const auto darcyElementIdx =
            this->problem(darcyIdx).gridGeometry().elementMapper().index(
                element);
        ...
        auto stokesFvGeometry =
            localView(this->problem(stokesIdx).gridGeometry());
        ...
        std::optional<ElementFluxVariablesCache<stokesIdx>>
            stokesElemFluxVarsCache;

        if (couplingMode() == CouplingMode::reconstructFreeFlowNormalStress) {
            curStokesElemVolVars.emplace(
                localView(gridVars_(stokesIdx).curGridVolVars()));
            curStokesElemFaceVars.emplace(
                localView(gridVars_(stokesIdx).curGridFaceVars()));
            prevStokesElemVolVars.emplace(
                localView(gridVars_(stokesIdx).prevGridVolVars()));
            prevStokesElemFaceVars.emplace(
                localView(gridVars_(stokesIdx).prevGridFaceVars()));
            stokesElemFluxVarsCache.emplace(
                localView(gridVars_(stokesIdx).gridFluxVarsCache()));
        }

        for (auto &&indices : stokesElementIndices) {
            const auto &stokesElement = this->problem(stokesIdx)
                                            .gridGeometry()
                                            .boundingBoxTree()
                                            .entitySet()
                                            .entity(indices.eIdx);
            ...
            if (couplingMode() ==
                ...
                using PriVarsType = typename VolumeVariables<
                    stokesCellCenterIdx>::PrimaryVariables;
                const auto &cellCenterPriVars =
                    this->curSol()[stokesCellCenterIdx][indices.eIdx];
                const auto elemSol =
                ...
                for (const auto &scv : scvs(stokesFvGeometry))
                    stokesVolVars->update(elemSol, this->problem(stokesIdx),
                                          stokesElement, scv);
                ...
                for (const auto &scvf : scvfs(stokesFvGeometry)) {
                    if (scvf.index() == indices.scvfIdx)
                        (*faceVelocity)[scvf.directionIndex()] =
                            this->curSol()[stokesFaceIdx][scvf.dofIndex()];
                }
            } else  // CouplingMode::reconstructFreeFlowNormalStress
            {
                curStokesElemVolVars->bind(stokesElement, stokesFvGeometry,
                                           this->curSol()[stokesCellCenterIdx]);
                curStokesElemFaceVars->bind(stokesElement, stokesFvGeometry,
                                            this->curSol()[stokesFaceIdx]);
                stokesElemFluxVarsCache->bind(stokesElement, stokesFvGeometry,
                                              *curStokesElemVolVars);
            }

            // add the context
            darcyCouplingContext_.push_back(
                {stokesElement, stokesFvGeometry, indices.scvfIdx,
                 indices.flipScvfIdx, faceVelocity, stokesVolVars,
                 curStokesElemVolVars, curStokesElemFaceVars,
                 prevStokesElemVolVars, prevStokesElemFaceVars,
                 stokesElemFluxVarsCache});
        }
    }
    ...
    void updateCouplingContext(
        Dune::index_constant<darcyIdx> domainI,
        const LocalAssemblerI &localAssemblerI,
        ...
        for (auto &data : darcyCouplingContext_) {
            ...
            if (couplingMode() ==
                ...
                for (const auto &scv : scvs(data.fvGeometry))
                    data.volVars->update(elemSol, this->problem(stokesIdx),
                                         data.element, scv);
            } else  // CouplingMode::reconstructFreeFlowNormalStress
            {
                for (const auto &scv : scvs(data.fvGeometry))
                    getVolVarAccess_(domainJ,
                                     gridVars_(stokesIdx).curGridVolVars(),
                                     data.curElemVolVars.value(), scv)
                        .update(elemSol, this->problem(stokesIdx), data.element,
                                scv);
            }
        }
    }
                               ...
                               int pvIdxJ)
    {
        ...
        for (auto &data : darcyCouplingContext_) {
            if (couplingMode() == CouplingMode::reconstructPorousMediumPressure)
                (*data.velocity)[data.getStokesScvf().directionIndex()] =
                    priVars;
            else  // CouplingMode::reconstructFreeFlowNormalStress
            {
                using FaceSolution =
                    GetPropType<SubDomainTypeTag<stokesIdx>,
                                Properties::StaggeredFaceSolution>;
                FaceSolution faceSol(data.getStokesScvf(),
                                     this->curSol()[domainJ],
                                     stokesProblem.gridGeometry());
                auto &faceVars = getFaceVarAccess_(
                    domainJ, gridVars_(stokesIdx).curGridFaceVars(),
                    data.curElemFaceVars.value(), data.getStokesScvf());
                faceVars.update(faceSol, stokesProblem, data.element,
                                data.fvGeometry, data.getStokesScvf());
            }
        }
    }
                               ...
                               int pvIdxJ)
    {
        ...
        for (auto &data : stokesCouplingContext_) {
            ...
            const auto darcyElemSol =
                elementSolution(data.element, this->curSol()[darcyIdx],
                                this->problem(darcyIdx).gridGeometry());

            for (const auto &scv : scvs(data.fvGeometry))
                data.volVars.update(darcyElemSol, this->problem(darcyIdx),
                                    data.element, scv);
        }
    }
    ...
    {
        elemVolVars.bind(localAssembler.element(), localAssembler.fvGeometry(),
        ...
        elemFluxVarsCache.update(localAssembler.element(),
                                 localAssembler.fvGeometry(), elemVolVars);
    }
    ...
    const CouplingStencil &couplingStencil(
        Dune::index_constant<stokesCellCenterIdx> domainI,
        const Element<stokesIdx> &element,
        Dune::index_constant<darcyIdx> domainJ) const
    {
        const auto eIdx =
            this->problem(domainI).gridGeometry().elementMapper().index(
                element);
     ...
     */
    const CouplingStencil &couplingStencil(
        Dune::index_constant<stokesFaceIdx> domainI,
        const SubControlVolumeFace<stokesIdx> &scvf,
        Dune::index_constant<darcyIdx> domainJ) const
    {
     ...
     */
    bool isCoupledEntity(Dune::index_constant<stokesIdx>,
                         const SubControlVolumeFace<stokesFaceIdx> &scvf) const
    {
     ...
     * \brief Returns whether a given free flow scvf is coupled to the other domain
     */
    bool isCoupledEntity(Dune::index_constant<darcyIdx>,
                         const SubControlVolumeFace<darcyIdx> &scvf) const
    {
    ...
    bool gridVariablesAvailable() const
    {
        ...
        forEach(
            std::make_index_sequence<std::tuple_size_v<GridVariablesTuple>>(),
            [&](const auto domainId) {
     ...
     */
    CouplingMode couplingMode() const { return couplingMode_; }
...
private:
    ...
    const GridVariables<i> &gridVars_(Dune::index_constant<i> domainIdx) const
    {
        if (std::get<i>(gridVariables_))
            ...
            DUNE_THROW(Dune::InvalidStateException,
                       "The gridVariables pointer was not set. Use "
                       "setGridVariables() before calling this function");
    }
    ...
    template<std::size_t i,
             std::enable_if_t<
                 !getPropValue<SubDomainTypeTag<i>,
                               Properties::EnableGridVolumeVariablesCache>(),
                 int> = 0>
    VolumeVariables<i> &getVolVarAccess_(Dune::index_constant<i> domainIdx,
    ...
    template<std::size_t i,
             std::enable_if_t<
                 !getPropValue<SubDomainTypeTag<i>,
                               Properties::EnableGridFaceVariablesCache>(),
                 int> = 0>
    FaceVariables<i> &getFaceVarAccess_(Dune::index_constant<i> domainIdx,
                                        GridFaceVariables<i> &gridFaceVariables,
                                        ElementFaceVariables<i> &elemFaceVars,
    ...
    void printCouplingMode_() const
    {
        const std::string mode =
            couplingMode() == CouplingMode::reconstructPorousMediumPressure
                ? "porous medium pressure"
                : "free-flow normal stress";
