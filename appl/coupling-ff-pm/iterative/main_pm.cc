    ...
    for (const auto &element : elements(fvGridGeometry.gridView())) {
        ...
        for (const auto &scvf : scvfs(fvGeometry)) {
            if (couplingInterface.isCoupledEntity(scvf.index())) {
                ...
                const double p = pressureAtInterface(
                    problem, element, fvGridGeometry, elemVolVars, scvf);
...
try {
    ...
    std::cout << dim << "  " << int(DarcyFVGridGeometry::GridView::dimension)
              << std::endl;
    ...
    const double xMin =
        getParamFromGroup<std::vector<double>>("Darcy", "Grid.Positions0")[0];
    const double xMax =
        getParamFromGroup<std::vector<double>>("Darcy", "Grid.Positions0")
            .back();
    ...
    auto assembler = std::make_shared<Assembler>(
        darcyProblem, darcyFvGridGeometry, darcyGridVariables);
