...
struct IsSameFluidSystem {
    static_assert(FFFS::numPhases == 1,
                  "Only single-phase fluidsystems may be used for free flow.");
    static constexpr bool value =
        std::is_same<typename FFFS::MultiPhaseFluidSystem, PMFS>::value;
};
...
struct IndexHelper<stokesIdx, darcyIdx, FFFS, false> {
    ...
    static constexpr auto couplingPhaseIdx(Dune::index_constant<i>,
     ...
     */
    template<std::size_t i>
    static constexpr auto couplingCompIdx(Dune::index_constant<i>,
                                          int coupledCompdIdx)
    {
...
struct IndexHelper<stokesIdx, darcyIdx, FFFS, true> {
     ...
     */
    static constexpr auto couplingPhaseIdx(Dune::index_constant<stokesIdx>,
                                           int coupledPhaseIdx = 0)
    {
        return 0;
    }
     ...
     */
    static constexpr auto couplingPhaseIdx(Dune::index_constant<darcyIdx>,
                                           int coupledPhaseIdx = 0)
    {
     ...
     */
    static constexpr auto couplingCompIdx(Dune::index_constant<stokesIdx>,
                                          int coupledCompdIdx)
    {
        return coupledCompdIdx;
    }
     ...
     */
    static constexpr auto couplingCompIdx(Dune::index_constant<darcyIdx>,
                                          int coupledCompdIdx)
    {
...
using StokesDarcyCouplingData = StokesDarcyCouplingDataImplementation<
    MDTraits,
    CouplingManager,
    GetPropType<typename MDTraits::template SubDomain<0>::TypeTag,
...
class StokesDarcyCouplingDataImplementationBase
{
    ...
    template<std::size_t id>
    using GridGeometry =
        GetPropType<SubDomainTypeTag<id>, Properties::GridGeometry>;
    template<std::size_t id>
    using Element =
        typename GridGeometry<id>::GridView::template Codim<0>::Entity;
    template<std::size_t id>
    using FVElementGeometry = typename GridGeometry<id>::LocalView;
    template<std::size_t id>
    ...
    template<std::size_t id>
    using GlobalPosition = typename Element<id>::Geometry::GlobalCoordinate;
    ...
    static constexpr bool adapterUsed =
        ModelTraits<darcyIdx>::numFluidPhases() > 1;
    using IndexHelper = Dumux::
        IndexHelper<stokesIdx, darcyIdx, FluidSystem<stokesIdx>, adapterUsed>;

    static constexpr int enableEnergyBalance =
        GetPropType<SubDomainTypeTag<stokesIdx>,
                    Properties::ModelTraits>::enableEnergyBalance();
    static_assert(
        GetPropType<SubDomainTypeTag<darcyIdx>,
                    Properties::ModelTraits>::enableEnergyBalance() ==
            enableEnergyBalance,
        "All submodels must both be either isothermal or non-isothermal");

    static_assert(
        IsSameFluidSystem<FluidSystem<stokesIdx>, FluidSystem<darcyIdx>>::value,
   ...
   public:
    StokesDarcyCouplingDataImplementationBase(
        ...
        : couplingManager_(couplingmanager), timeLoop_(timeLoop)
    {
    }
     ...
     */
    template<std::size_t i>
    static constexpr auto couplingPhaseIdx(Dune::index_constant<i> id,
                                           int coupledPhaseIdx = 0)
    {
     ...
     */
    template<std::size_t i>
    static constexpr auto couplingCompIdx(Dune::index_constant<i> id,
                                          int coupledCompdIdx)
    {
     ...
     */
    auto darcyPermeability(const Element<stokesIdx> &element,
    ...
    Scalar momentumCouplingCondition(
        ...
        const SubControlVolumeFace<stokesIdx> &scvf) const
    {
        if (couplingManager().couplingMode() !=
            CouplingManager::CouplingMode::reconstructPorousMediumPressure)
            DUNE_THROW(Dune::InvalidStateException,
                       "This condition can only be used if the coupling mode "
                       "is set to reconstructPorousMediumPressure");

        static constexpr auto numPhasesDarcy =
            GetPropType<SubDomainTypeTag<darcyIdx>,
                        Properties::ModelTraits>::numFluidPhases();

        Scalar momentumFlux(0.0);
        const auto &stokesContext =
            couplingManager_.stokesCouplingContext(element, scvf);
        const auto darcyPhaseIdx = couplingPhaseIdx(darcyIdx);

        // - p_pm * n_pm = p_pm * n_ff
        const Scalar darcyPressure =
            stokesContext.volVars.pressure(darcyPhaseIdx);

        if (numPhasesDarcy > 1)
            momentumFlux = darcyPressure;
        else  // use pressure reconstruction for single phase models
            momentumFlux = pressureAtInterface_(
                element, scvf, stokesElemFaceVars, stokesContext);
        // TODO: generalize for permeability tensors

        // normalize pressure
        if (getPropValue<SubDomainTypeTag<stokesIdx>,
                         Properties::NormalizePressure>())
            momentumFlux -= couplingManager_.problem(stokesIdx).initial(
                scvf)[Indices<stokesIdx>::pressureIdx];
     ...
     */
    Scalar freeFlowInterfaceStress(
        const Element<darcyIdx> &element,
        const SubControlVolumeFace<darcyIdx> &scvf) const
    {
        if (couplingManager().couplingMode() !=
            CouplingManager::CouplingMode::reconstructFreeFlowNormalStress)
            DUNE_THROW(Dune::InvalidStateException,
                       "This condition can only be used if the coupling mode "
                       "is set to reconstructFreeFlowNormalStress");
        ...
        const auto &curFreeFlowElemVolVars =
            darcyContext.curElemVolVars.value();
        const auto &prevFreeFlowElemVolVars =
            darcyContext.prevElemVolVars.value();
        const auto &freeFlowProblem =
            this->couplingManager().problem(stokesIdx);
        const auto &freeFlowElement = darcyContext.element;
        const auto &freeFlowFVGeometry = darcyContext.fvGeometry;
        const auto &curFreeFlowElemenFaceVars =
            darcyContext.curElemFaceVars.value();
        const auto &prevFreeFlowElemenFaceVars =
            darcyContext.prevElemFaceVars.value();

        using LocalResidual =
            GetPropType<SubDomainTypeTag<stokesIdx>, Properties::LocalResidual>;
        LocalResidual localResidual(&freeFlowProblem, timeLoop_.get());

        typename LocalResidual::FaceResidualValue result(0.0);

        if (timeLoop_)
            localResidual.evalStorageForFace(
                result, freeFlowProblem, freeFlowElement, freeFlowFVGeometry,
                prevFreeFlowElemVolVars, curFreeFlowElemVolVars,
                prevFreeFlowElemenFaceVars, curFreeFlowElemenFaceVars,
                freeFlowScvf);

        localResidual.evalSourceForFace(
            result, freeFlowProblem, freeFlowElement, freeFlowFVGeometry,
            curFreeFlowElemVolVars, curFreeFlowElemenFaceVars, freeFlowScvf);

        // consider momentum flux
        using FluxVariables =
            GetPropType<SubDomainTypeTag<stokesIdx>, Properties::FluxVariables>;
        FluxVariables fluxVars;
        result += fluxVars.computeMomentumFlux(
            freeFlowProblem, freeFlowElement, freeFlowScvf, freeFlowFVGeometry,
            curFreeFlowElemVolVars, curFreeFlowElemenFaceVars,
            darcyContext.elemFluxVarsCache->gridFluxVarsCache());

        result /= curFreeFlowElemVolVars[freeFlowScvf.insideScvIdx()]
                      .extrusionFactor() *
                  freeFlowScvf.area();
        return result;
    }

    Scalar freeFlowInterfacePressure(
        const Element<darcyIdx> &element,
        const SubControlVolumeFace<darcyIdx> &scvf) const
    {
        if (couplingManager().couplingMode() !=
     ...
     *        [v*n]^FF = [v*n]^PM
     */
    Scalar porousMediumInterfaceVelocity(
        const Element<stokesIdx> &element,
        const SubControlVolumeFace<stokesIdx> &scvf) const
    {
        static_assert(
            std::is_same_v<AdvectionType, DarcysLaw>,
            "This function is currently only implemented for Darcy's law");
        ...
        const Scalar couplingPhaseCellCenterPressure =
            stokesContext.volVars.pressure(darcyPhaseIdx);
        const Scalar couplingPhaseInterfacePressure = freeFlowInterfaceStress(
            stokesContext.element, stokesContext.getDarcyScvf());
        const Scalar couplingPhaseMobility =
            stokesContext.volVars.mobility(darcyPhaseIdx);
        const Scalar couplingPhaseDensity =
            stokesContext.volVars.density(darcyPhaseIdx);
        const auto K = stokesContext.volVars.permeability();

        const auto &insideScv =
            stokesContext.fvGeometry.scv(darcyScvf.insideScvIdx());
        const auto ti =
            computeTpfaTransmissibility(darcyScvf, insideScv, K, 1.0);
        const auto alpha =
            vtmv(darcyScvf.unitOuterNormal(), K,
                 couplingManager_.problem(darcyIdx).spatialParams().gravity(
                     darcyScvf.center()));

        return couplingPhaseMobility * (ti * (couplingPhaseCellCenterPressure -
                                              couplingPhaseInterfacePressure) +
                                        couplingPhaseDensity * alpha);
    }

    /*!
     * \brief Evaluate an advective flux across the interface and consider upwinding.
     */
    Scalar advectiveFlux(const Scalar insideQuantity,
                         const Scalar outsideQuantity,
                         const Scalar volumeFlow,
                         bool insideIsUpstream) const
    {
        const Scalar upwindWeight =
            1.0;  //TODO use Flux.UpwindWeight or something like Coupling.UpwindWeight?

        if (insideIsUpstream)
            return (upwindWeight * insideQuantity +
                    (1.0 - upwindWeight) * outsideQuantity) *
                   volumeFlow;
        else
            return (upwindWeight * outsideQuantity +
                    (1.0 - upwindWeight) * insideQuantity) *
                   volumeFlow;
    }
   ...
   protected:
        ...
        const DiffusionCoefficientAveragingType diffCoeffAvgType) const
    {
        ...
        if (diffCoeffAvgType == DiffusionCoefficientAveragingType::harmonic) {
            ...
            return domainI == stokesIdx ? avgQuantityI / totalDistance
                                        : avgQuantityJ / totalDistance;

        else  // diffCoeffAvgType == DiffusionCoefficientAveragingType::pmOnly)
            return domainI == darcyIdx ? avgQuantityI / totalDistance
                                       : avgQuantityJ / totalDistance;
    }
        ...
        const DiffusionCoefficientAveragingType diffCoeffAvgType) const
    {
        ...
        const Scalar tij = transmissibility_(
            domainI, domainJ, insideDistance, outsideDistance,
            volVarsI.effectiveThermalConductivity(),
            volVarsJ.effectiveThermalConductivity(), diffCoeffAvgType);
                                ...
                                const CouplingContext &context) const
    {
        ...
        return computeCouplingPhasePressureAtInterface_(
            context.element, context.fvGeometry, darcyScvf, context.volVars,
            velocity, AdvectionType());
    }

    /*!
     * \brief Returns the pressure at the interface using Forchheimers's law for reconstruction
     */
    Scalar computeCouplingPhasePressureAtInterface_(
        const Element<darcyIdx> &element,
        ...
        const typename Element<stokesIdx>::Geometry::GlobalCoordinate
        ...
        ForchheimersLaw) const
    {
        ...
        Scalar cF =
            couplingManager_.problem(darcyIdx).spatialParams().forchCoeff(scvf);

        const Scalar interfacePressure =
            ((-mu * (scvf.unitOuterNormal() * velocity)) +
             (-(scvf.unitOuterNormal() * velocity) * velocity.two_norm() * rho *
              sqrt(darcyPermeability(element, scvf)) * cF) +
             rho * alpha) /
                ti +
            cellCenterPressure;
        ...
        DarcysLaw) const
    {
        const auto darcyPhaseIdx = couplingPhaseIdx(darcyIdx);
        const Scalar couplingPhaseCellCenterPressure =
            volVars.pressure(darcyPhaseIdx);
        ...
        return (-1 / couplingPhaseMobility *
                    (scvf.unitOuterNormal() * couplingPhaseVelocity) +
                couplingPhaseDensity * alpha) /
                   ti +
...
class StokesDarcyCouplingDataImplementation<MDTraits,
                                            CouplingManager,
                                            enableEnergyBalance,
                                            false>
    : public StokesDarcyCouplingDataImplementationBase<MDTraits,
                                                       CouplingManager>
{
    using ParentType =
        StokesDarcyCouplingDataImplementationBase<MDTraits, CouplingManager>;
    using Scalar = typename MDTraits::Scalar;
    static constexpr auto stokesIdx = CouplingManager::stokesIdx;
    static constexpr auto darcyIdx = CouplingManager::darcyIdx;
    static constexpr auto stokesFaceIdx = CouplingManager::stokesFaceIdx;
    static constexpr auto stokesCellCenterIdx =
        CouplingManager::stokesCellCenterIdx;
    ...
    static_assert(GetPropType<SubDomainTypeTag<darcyIdx>,
                              Properties::ModelTraits>::numFluidComponents() ==
                      GetPropType<SubDomainTypeTag<darcyIdx>,
                                  Properties::ModelTraits>::numFluidPhases(),
   ...
   public:
        ...
        const SubControlVolumeFace<darcyIdx> &scvf) const
    {
        ...
        const Scalar velocity =
            darcyContext.velocity.value() * scvf.unitOuterNormal();
        const Scalar darcyDensity =
            darcyElemVolVars[scvf.insideScvIdx()].density(
                couplingPhaseIdx(darcyIdx));

        const auto &stokesScvf = darcyContext.getStokesScvf();
        const Scalar stokesDensity =
            this->couplingManager().couplingMode() ==
                    CouplingManager::CouplingMode::
                        reconstructPorousMediumPressure
                ? darcyContext.volVars.value().density()
                : darcyContext.curElemVolVars.value()[stokesScvf.insideScvIdx()]
                      .density();

        const bool insideIsUpstream = velocity > 0.0;

        return massFlux_(velocity, darcyDensity, stokesDensity,
                         insideIsUpstream);
    }
        ...
        const SubControlVolumeFace<stokesIdx> &scvf) const
    {
        ...
        const Scalar stokesDensity =
            stokesElemVolVars[scvf.insideScvIdx()].density();
        const Scalar darcyDensity =
            stokesContext.volVars.density(couplingPhaseIdx(darcyIdx));
        const bool insideIsUpstream = sign(velocity) == scvf.directionSign();

        return massFlux_(velocity * scvf.directionSign(), stokesDensity,
                         darcyDensity, insideIsUpstream);
    }
            ...
            DiffusionCoefficientAveragingType::ffOnly) const
    {
        ...
        return energyFlux_(darcyIdx, stokesIdx, fvGeometry,
                           darcyContext.fvGeometry, scvf, darcyVolVars,
                           stokesVolVars, velocity, insideIsUpstream,
                           diffCoeffAvgType);
    }
            ...
            DiffusionCoefficientAveragingType::ffOnly) const
    {
        ...
        return energyFlux_(stokesIdx, darcyIdx, fvGeometry,
                           stokesContext.fvGeometry, scvf, stokesVolVars,
                           darcyVolVars, velocity * scvf.directionSign(),
   ...
   private:
    ...
    Scalar massFlux_(const Scalar velocity,
                     ...
                     bool insideIsUpstream) const
    {
        return this->advectiveFlux(insideDensity, outSideDensity, velocity,
                                   insideIsUpstream);
    }
        ...
        const DiffusionCoefficientAveragingType diffCoeffAvgType) const
    {
        ...
        // convective fluxes
        const Scalar insideTerm =
            insideVolVars.density(couplingPhaseIdx(domainI)) *
            insideVolVars.enthalpy(couplingPhaseIdx(domainI));
        const Scalar outsideTerm =
            outsideVolVars.density(couplingPhaseIdx(domainJ)) *
            outsideVolVars.enthalpy(couplingPhaseIdx(domainJ));

        flux += this->advectiveFlux(insideTerm, outsideTerm, velocity,
                                    insideIsUpstream);
                                                       ...
                                                       CouplingManager>
{
    ...
    static constexpr auto numComponents =
        GetPropType<SubDomainTypeTag<stokesIdx>,
                    Properties::ModelTraits>::numFluidComponents();
    static constexpr auto replaceCompEqIdx =
        GetPropType<SubDomainTypeTag<stokesIdx>,
                    Properties::ModelTraits>::replaceCompEqIdx();
    static constexpr bool useMoles =
        GetPropType<SubDomainTypeTag<stokesIdx>,
                    Properties::ModelTraits>::useMoles();
    static constexpr auto referenceSystemFormulation = GetPropType<
        SubDomainTypeTag<stokesIdx>,
        Properties::MolecularDiffusionType>::referenceSystemFormulation();

    static_assert(GetPropType<SubDomainTypeTag<darcyIdx>,
                              Properties::ModelTraits>::numFluidComponents() ==
                      numComponents,
                  "Both submodels must use the same number of components");
    static_assert(
        getPropValue<SubDomainTypeTag<darcyIdx>, Properties::UseMoles>() ==
            useMoles,
        "Both submodels must either use moles or not");
    static_assert(getPropValue<SubDomainTypeTag<darcyIdx>,
                               Properties::ReplaceCompEqIdx>() ==
                      replaceCompEqIdx,
    ...
    static_assert(GetPropType<SubDomainTypeTag<darcyIdx>,
                      ...
                      referenceSystemFormulation,
                  "Both submodels must use the same reference system "
    ...
    static constexpr bool isFicksLaw =
        IsFicksLaw<GetPropType<SubDomainTypeTag<stokesIdx>,
                               Properties::MolecularDiffusionType>>();
    static_assert(
        isFicksLaw ==
            IsFicksLaw<GetPropType<SubDomainTypeTag<darcyIdx>,
                                   Properties::MolecularDiffusionType>>(),
        "Both submodels must use the same diffusion law.");

    using ReducedComponentVector = Dune::FieldVector<Scalar, numComponents - 1>;
    using ReducedComponentMatrix =
        Dune::FieldMatrix<Scalar, numComponents - 1, numComponents - 1>;

    using MolecularDiffusionType =
        GetPropType<SubDomainTypeTag<stokesIdx>,
                    Properties::MolecularDiffusionType>;
   ...
   public:
     ...
     * \brief Returns the mass flux across the coupling boundary as seen from the Darcy domain.
     */
    NumEqVector massCouplingCondition(
        const Element<darcyIdx> &element,
        const FVElementGeometry<darcyIdx> &fvGeometry,
        const ElementVolumeVariables<darcyIdx> &darcyElemVolVars,
        const SubControlVolumeFace<darcyIdx> &scvf,
            ...
            DiffusionCoefficientAveragingType::ffOnly) const
    {
        ...
        return massFlux_(darcyIdx, stokesIdx, fvGeometry, scvf, darcyVolVars,
                         stokesVolVars, outsideScv, velocity, insideIsUpstream,
                         diffCoeffAvgType);
    }
            ...
            DiffusionCoefficientAveragingType::ffOnly) const
    {
        ...
        return massFlux_(stokesIdx, darcyIdx, fvGeometry, scvf, stokesVolVars,
                         darcyVolVars, outsideScv,
   ...
   protected:
        ...
        const DiffusionCoefficientAveragingType diffCoeffAvgType) const
    {
        ...
        auto moleOrMassFraction = [&](const auto &volVars, int phaseIdx,
                                      int compIdx) {
        ...
        auto moleOrMassDensity = [&](const auto &volVars, int phaseIdx) {
            return useMoles ? volVars.molarDensity(phaseIdx)
                            : volVars.density(phaseIdx);
        };

        // treat the advective fluxes
        auto insideTerm = [&](int compIdx) {
            return moleOrMassFraction(insideVolVars, couplingPhaseIdx(domainI),
                                      compIdx) *
                   moleOrMassDensity(insideVolVars, couplingPhaseIdx(domainI));
        };

        auto outsideTerm = [&](int compIdx) {
            return moleOrMassFraction(outsideVolVars, couplingPhaseIdx(domainJ),
                                      compIdx) *
                   moleOrMassDensity(outsideVolVars, couplingPhaseIdx(domainJ));
        };
        ...
        for (int compIdx = 0; compIdx < numComponents; ++compIdx) {
            ...
            flux[domainICompIdx] += this->advectiveFlux(
                insideTerm(domainICompIdx), outsideTerm(domainJCompIdx),
                velocity, insideIsUpstream);
        }
        ...
        if (isFicksLaw)
            diffusiveFlux += diffusiveMolecularFluxFicksLaw_(
                domainI, domainJ, scvf, insideScv, outsideScv, insideVolVars,
                outsideVolVars, diffCoeffAvgType);
        else  //maxwell stefan
            diffusiveFlux += diffusiveMolecularFluxMaxwellStefan_(
                domainI, domainJ, scvf, insideScv, outsideScv, insideVolVars,
                outsideVolVars);
        ...
        if (referenceSystemFormulation ==
            ...
            for (int compIdx = 0; compIdx < numComponents; ++compIdx) {
                const int domainICompIdx = couplingCompIdx(domainI, compIdx);
                diffusiveFlux[domainICompIdx] *=
                    1 / FluidSystem<i>::molarMass(domainICompIdx);
            }
        }
        if (referenceSystemFormulation ==
            ...
            for (int compIdx = 0; compIdx < numComponents; ++compIdx) {
                const int domainICompIdx = couplingCompIdx(domainI, compIdx);
                diffusiveFlux[domainICompIdx] *=
                    FluidSystem<i>::molarMass(domainICompIdx);
            }
        }
        ...
        if (replaceCompEqIdx < numComponents)
            flux[replaceCompEqIdx] =
                std::accumulate(flux.begin(), flux.end(), 0.0);

        return flux;
    }

    Scalar getComponentEnthalpy(const VolumeVariables<stokesIdx> &volVars,
                                int phaseIdx,
                                int compIdx) const
    {
        return FluidSystem<stokesIdx>::componentEnthalpy(volVars.fluidState(),
                                                         0, compIdx);
    }

    Scalar getComponentEnthalpy(const VolumeVariables<darcyIdx> &volVars,
                                int phaseIdx,
                                int compIdx) const
    {
        return FluidSystem<darcyIdx>::componentEnthalpy(volVars.fluidState(),
                                                        phaseIdx, compIdx);
    }
     ...
     */
    template<std::size_t i, std::size_t j>
    NumEqVector diffusiveMolecularFluxMaxwellStefan_(
        Dune::index_constant<i> domainI,
        Dune::index_constant<j> domainJ,
        const SubControlVolumeFace<i> &scvfI,
        const SubControlVolume<i> &scvI,
        ...
        const VolumeVariables<j> &volVarsJ) const
    {
        ...
        for (int compIdx = 0; compIdx < numComponents - 1; compIdx++) {
            ...
            assert(FluidSystem<i>::componentName(domainICompIdx) ==
                   FluidSystem<j>::componentName(domainJCompIdx));

            //calculate x_inside
            const Scalar xInside = volVarsI.moleFraction(
                couplingPhaseIdx(domainI), domainICompIdx);
            //calculate outside molefraction with the respective transmissibility
            const Scalar xOutside = volVarsJ.moleFraction(
                couplingPhaseIdx(domainJ), domainJCompIdx);
        ...
        for (int compKIdx = 0; compKIdx < numComponents - 1; compKIdx++) {
            ...
            const Scalar xk = volVarsI.moleFraction(couplingPhaseIdx(domainI),
                                                    domainICompKIdx);
            const Scalar avgMolarMass =
                volVarsI.averageMolarMass(couplingPhaseIdx(domainI));
            const Scalar Mn = FluidSystem<i>::molarMass(numComponents - 1);
            const Scalar tkn = volVarsI.effectiveDiffusionCoefficient(
                couplingPhaseIdx(domainI), domainICompKIdx,
                couplingCompIdx(domainI, numComponents - 1));

            // set the entries of the diffusion matrix of the diagonal
            reducedDiffusionMatrixInside[domainICompKIdx][domainICompKIdx] +=
                xk * avgMolarMass / (tkn * Mn);
            ...
            for (int compLIdx = 0; compLIdx < numComponents; compLIdx++) {
                ...
                const Scalar xl = volVarsI.moleFraction(
                    couplingPhaseIdx(domainI), domainICompLIdx);
                const Scalar Mk = FluidSystem<i>::molarMass(domainICompKIdx);
                const Scalar Ml = FluidSystem<i>::molarMass(domainICompLIdx);
                const Scalar tkl = volVarsI.effectiveDiffusionCoefficient(
                    couplingPhaseIdx(domainI), domainICompKIdx,
                    domainICompLIdx);
                reducedDiffusionMatrixInside[domainICompKIdx]
                                            [domainICompKIdx] +=
                    xl * avgMolarMass / (tkl * Mk);
                reducedDiffusionMatrixInside[domainICompKIdx]
                                            [domainICompLIdx] +=
                    xk *
                    (avgMolarMass / (tkn * Mn) - avgMolarMass / (tkl * Ml));
            }
        }
        //outside matrix
        for (int compKIdx = 0; compKIdx < numComponents - 1; compKIdx++) {
            ...
            const Scalar xk = volVarsJ.moleFraction(couplingPhaseIdx(domainJ),
                                                    domainJCompKIdx);
            const Scalar avgMolarMass =
                volVarsJ.averageMolarMass(couplingPhaseIdx(domainJ));
            const Scalar Mn = FluidSystem<j>::molarMass(numComponents - 1);
            const Scalar tkn = volVarsJ.effectiveDiffusionCoefficient(
                couplingPhaseIdx(domainJ), domainJCompKIdx,
                couplingCompIdx(domainJ, numComponents - 1));

            // set the entries of the diffusion matrix of the diagonal
            reducedDiffusionMatrixOutside[domainICompKIdx][domainICompKIdx] +=
                xk * avgMolarMass / (tkn * Mn);
            ...
            for (int compLIdx = 0; compLIdx < numComponents; compLIdx++) {
                ...
                const Scalar xl = volVarsJ.moleFraction(
                    couplingPhaseIdx(domainJ), domainJCompLIdx);
                const Scalar Mk = FluidSystem<j>::molarMass(domainJCompKIdx);
                const Scalar Ml = FluidSystem<j>::molarMass(domainJCompLIdx);
                const Scalar tkl = volVarsJ.effectiveDiffusionCoefficient(
                    couplingPhaseIdx(domainJ), domainJCompKIdx,
                    domainJCompLIdx);
                reducedDiffusionMatrixOutside[domainICompKIdx]
                                             [domainICompKIdx] +=
                    xl * avgMolarMass / (tkl * Mk);
                reducedDiffusionMatrixOutside[domainICompKIdx]
                                             [domainICompLIdx] +=
        ...
        reducedDiffusionMatrixInside *=
            omegai * volVarsI.density(couplingPhaseIdx(domainI));
        reducedDiffusionMatrixOutside.invert();
        reducedDiffusionMatrixOutside *=
            omegaj * volVarsJ.density(couplingPhaseIdx(domainJ));
        ...
        for (int compIdx = 0; compIdx < numComponents - 1; compIdx++) {
            ...
            diffusiveFlux[couplingCompIdx(domainI, numComponents - 1)] -=
                reducedFlux[domainICompIdx];
        }
        return diffusiveFlux;
    }

    template<std::size_t i, std::size_t j>
    NumEqVector diffusiveMolecularFluxFicksLaw_(
        Dune::index_constant<i> domainI,
        Dune::index_constant<j> domainJ,
        const SubControlVolumeFace<i> &scvfI,
        const SubControlVolume<i> &scvI,
        ...
        const DiffusionCoefficientAveragingType diffCoeffAvgType) const
    {
        ...
        const Scalar rhoInside = massOrMolarDensity(
            volVarsI, referenceSystemFormulation, couplingPhaseIdx(domainI));
        const Scalar rhoOutside = massOrMolarDensity(
            volVarsJ, referenceSystemFormulation, couplingPhaseIdx(domainJ));
        ...
        for (int compIdx = 1; compIdx < numComponents; ++compIdx) {
            ...
            const Scalar massOrMoleFractionInside =
                massOrMoleFraction(volVarsI, referenceSystemFormulation,
                                   couplingPhaseIdx(domainI), domainICompIdx);
            const Scalar massOrMoleFractionOutside =
                massOrMoleFraction(volVarsJ, referenceSystemFormulation,
                                   couplingPhaseIdx(domainJ), domainJCompIdx);

            const Scalar deltaMassOrMoleFrac =
                massOrMoleFractionOutside - massOrMoleFractionInside;
            const Scalar tij = this->transmissibility_(
                domainI, domainJ, insideDistance, outsideDistance,
                volVarsI.effectiveDiffusionCoefficient(
                    couplingPhaseIdx(domainI), domainIMainCompIdx,
                    domainICompIdx),
                volVarsJ.effectiveDiffusionCoefficient(
                    couplingPhaseIdx(domainJ), domainJMainCompIdx,
                    domainJCompIdx),
                diffCoeffAvgType);
            diffusiveFlux[domainICompIdx] +=
                -avgDensity * tij * deltaMassOrMoleFrac;
        }

        const Scalar cumulativeFlux =
            std::accumulate(diffusiveFlux.begin(), diffusiveFlux.end(), 0.0);
        ...
        const DiffusionCoefficientAveragingType diffCoeffAvgType) const
    {
        ...
        auto diffusiveFlux =
            isFicksLaw ? diffusiveMolecularFluxFicksLaw_(
                             domainI, domainJ, scvf, insideScv, outsideScv,
                             insideVolVars, outsideVolVars, diffCoeffAvgType)
        ...
        for (int compIdx = 0; compIdx < diffusiveFlux.size(); ++compIdx) {
            ...
            const bool insideDiffFluxIsUpstream =
                diffusiveFlux[domainICompIdx] > 0;
            const Scalar componentEnthalpy =
                insideDiffFluxIsUpstream
                    ? getComponentEnthalpy(insideVolVars,
                                           couplingPhaseIdx(domainI),
                                           domainICompIdx)
                    : getComponentEnthalpy(outsideVolVars,
                                           couplingPhaseIdx(domainJ),
                                           domainJCompIdx);
            ...
            if (referenceSystemFormulation ==
                ...
                flux += diffusiveFlux[domainICompIdx] *
                        FluidSystem<i>::molarMass(domainICompIdx) *
                        componentEnthalpy;
        }
