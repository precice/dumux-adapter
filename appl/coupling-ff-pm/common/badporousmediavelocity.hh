...
#ifndef DUMUX_BADPOROUSMEDIUMFLOW_VELOCITY_HH
#define DUMUX_BADPOROUSMEDIUMFLOW_VELOCITY_HH
...
class BadPorousMediumFlowVelocity
{
    ...
    static constexpr bool stationaryVelocityField =
        FluxTraits::hasStationaryVelocityField();
   ...
   public:
     ...
     */
    BadPorousMediumFlowVelocity(const GridVariables &gridVariables)
          ...
          gridVariables_(gridVariables)
    {
        ...
        if constexpr (isBox && dim > 1) {
            ...
            for (const auto &element : elements(gridGeometry_.gridView()))
                for (unsigned int vIdx = 0; vIdx < element.subEntities(dim);
                     ++vIdx)
                    ++cellNum_[gridGeometry_.vertexMapper().subIndex(
                        element, vIdx, dim)];
        }
    }
    ...
    void calculateVelocity(VelocityVector &velocity,
                           ...
                           int phaseIdx) const
    {
        std::cout << "BadPorousMediaVelocity" << std::endl;
        ...
        if constexpr (isBox && dim == 1) {
            ...
            for (auto &&scvf : scvfs(fvGeometry)) {
                ...
                fluxVars.init(problem_, element, fvGeometry, elemVolVars, scvf,
                              elemFluxVarsCache);
                ...
                Scalar flux =
                    fluxVars.advectiveFlux(phaseIdx, upwindTerm) / localArea;
                const auto &insideVolVars = elemVolVars[scvf.insideScvIdx()];
                flux /= insideVolVars.extrusionFactor();
                tmpVelocity *= flux;

                const int eIdxGlobal =
        ...
        if constexpr (isBox) {
            ...
            for (auto &&scv : scvs(fvGeometry)) {
                ...
                jacobianT2.mtv(scvVelocities[scv.indexInElement()],
                               scvVelocity);
            ...
            static constexpr bool isMpfa =
                GridGeometry::discMethod == DiscretizationMethod::ccmpfa;
            const int numScvfsPerFace =
                isMpfa ? element.template subEntity<1>(0).geometry().corners()
                       : 1;

            if (fvGeometry.numScvf() !=
                element.subEntities(1) * numScvfsPerFace)
                DUNE_THROW(Dune::NotImplemented,
                           "Velocity output for non-conforming grids");

            if (!geomType.isCube() && !geomType.isSimplex())
                DUNE_THROW(Dune::NotImplemented,
                           "Velocity output for other geometry types than cube "
                           "and simplex");
            ...
            for (const auto &intersection :
                ...
                if (intersection.neighbor() || intersection.boundary()) {
                    for (int i = 0; i < numScvfsPerFace; ++i)
                        scvfIndexInInside[localScvfIdx++] =
            ...
            for (auto &&scvf : scvfs(fvGeometry)) {
                ...
                if (!scvf.boundary()) {
                    FluxVariables fluxVars;
                    fluxVars.init(problem_, element, fvGeometry, elemVolVars,
                                  scvf, elemFluxVarsCache);
                    scvfFluxes[scvfIndexInInside[localScvfIdx]] +=
                ...
                if (scvf.boundary()) {
                    ...
                    if (bcTypes.hasNeumann()) {
                        ...
                        if (stationaryVelocityField) {
                            const auto flux = AdvectionType::flux(
                                problem_, element, fvGeometry, elemVolVars,
                            ...
                            const auto neumannFlux = problem_.neumann(
                                element, fvGeometry, elemVolVars,
                                elemFluxVarsCache, scvf);
                            using NumEqVector =
                                std::decay_t<decltype(neumannFlux)>;
                            if (Dune::FloatCmp::eq<
                                    NumEqVector,
                                    Dune::FloatCmp::CmpStyle::absolute>(
                                    neumannFlux, NumEqVector(0.0), 1e-30))
                            ...
                            // for cubes
                                ...
                                const auto fIdx =
                                    scvfIndexInInside[localScvfIdx];
                                const auto fIdxOpposite =
                                    fIdx % 2 ? fIdx - 1 : fIdx + 1;
                            ...
                            // for simplices
                                ...
                                DUNE_THROW(Dune::NotImplemented,
                                           "Velocity computation at Neumann "
                                           "boundaries for cell-centered and "
                                           "prism/pyramid");
                        }
                    }
                }
            ...
            if (dim == 1 || geomType.isCube()) {
                for (int i = 0; i < dim; i++)
                    refVelocity[i] =
                        0.5 * (scvfFluxes[2 * i + 1] - scvfFluxes[2 * i]);
            }
   ...
   private:
    ...
    static Scalar scvfReferenceArea_(Dune::GeometryType geomType, int fIdx)
    {
        if (dim == 1 || geomType.isCube()) {
            ...
            static const Scalar faceToArea[] = {
                0.372677996249965, 0.372677996249965, 0.235702260395516};
            return faceToArea[fIdx];
        } else if (geomType.isTetrahedron()) {
            static const Scalar faceToArea[] = {
                0.102062072615966, 0.102062072615966, 0.058925565098879,
                0.102062072615966, 0.058925565098879, 0.058925565098879};
            return faceToArea[fIdx];
        } else if (geomType.isPyramid()) {
            static const Scalar faceToArea[] = {
                0.130437298687488, 0.130437298687488, 0.130437298687488,
                0.130437298687488, 0.150923085635624, 0.1092906420717,
                0.1092906420717,   0.0781735959970571};
            return faceToArea[fIdx];
        } else if (geomType.isPrism()) {
            static const Scalar faceToArea[] = {
                0.166666666666667, 0.166666666666667, 0.166666666666667,
                0.186338998124982, 0.186338998124982, 0.117851130197758,
                0.186338998124982, 0.186338998124982, 0.117851130197758};
            return faceToArea[fIdx];
        } else {
            DUNE_THROW(Dune::NotImplemented,
