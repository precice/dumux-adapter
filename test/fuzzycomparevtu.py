...
def compare_vtk(
    vtk1, vtk2, absolute=1.5e-7, relative=1e-2, zeroValueThreshold={}, verbose=True
):
...
def convert_pvtu_to_vtu(pvturoot, filename):
    ...
    for piece in pieces[1:]:
        ...
        newNumPoints = (
            int(piece.findall(".//Piece")[0].attrib["NumberOfPoints"]) + rootNumPoints
        )
        newNumCells = (
            int(piece.findall(".//Piece")[0].attrib["NumberOfCells"]) + rootNumCells
        )
...
def is_fuzzy_equal_node(
    ...
    for node1child, node2child in zip(node1.iter(), node2.iter()):
        if node1.tag != node2.tag:
            if verbose:
                print(
                    "The name of the node differs in: {} and {}".format(
                        node1.tag, node2.tag
                    )
                )
                is_equal = False
    ...
    for i in range(0, numComp):
        ...
        if numComp > 1:
            ...
            if parameter in zeroValueThreshold:
                zeroValueThreshold["{}_{}".format(parameter, i)] = zeroValueThreshold[
                    parameter
                ]
        else:
                ...
                if verbose:
                    ...
                    if largernumber != 0.0:
                        if diff / largernumber > max_relative_difference:
                            ...
                            message = "Difference is too large: {:.6%} -> between: {} and {}".format(
    ...
    if not is_fuzzy_equal_node(
        ...
        for root in [root1, root2]:
            ...
            if convertedFromParallelVtu:
                ...
                for i in range(len(coords) // dim):
                    vertexArray.append(
                        [float(c) for c in coords[i * dim : i * dim + dim]]
                    )
            ...
            for cellIdx, cell in enumerate(cellArray):
                ...
                midpoint = [
                    i / float(len(coords))
                    for i in [sum(coord) for coord in zip(*coords)]
                ]
            ...
            epsilon = (
                math.sqrt(sum([(a - b) ** 2 for a, b in zip(bBoxMax, bBoxMin)])) * 1e-7
            )
            ...
            def vertex_cmp(a, b):
                ...
                res2 = floatvec_cmp(
                    largestCellMidPointForVertex[a[0]],
                    largestCellMidPointForVertex[b[0]],
                    epsilon,
                )
                if res2 != 0:
                    return res2
            ...
            for idxNew, idxOld in enumerate(
                sorted(vMap, key=functools.cmp_to_key(vertex_cmp))
            ):
            ...
            for name, text in list(dataArrays.items()):
                ...
                if name in pointDataArrays:
                    ...
                    sortedItems = [
                        j for (i, j) in sorted(zip(cellArray, items), key=itemgetter(0))
                    ]
                elif name == "offsets":
                    sortedItems = []
                    counter = 0
